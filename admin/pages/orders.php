...
// Get orders data from sample data
$page = $_GET['p'] ?? 1;
$search = $_GET['search'] ?? '';
$status = $_GET['status'] ?? '';
...
$ordersData = makeApiCall("/admin/recent-orders");
$allOrders = $ordersData['success'] ? $ordersData['data'] : [];
...
if ($_POST && isset($_POST['action']) && $_POST['action'] === 'update_status' && $orderId) {
    ...
    $result = makeApiCall("/admin/orders/$orderId/status", 'PUT', ['status' => $newStatus]);
...
if ($action === 'view' && $orderId) {
    $orderResult = makeApiCall("/admin/orders/$orderId");
